...
fetch('https://cats.petiteweb.dev/api/single/WtfManOmg/show/')
  .then((res) => res.json())
  .then((data) => {
...
$wr.addEventListener('click', (e) => {
  if (e.target.dataset.action === ACTIONS.DELETE) {
    ...
    fetch(`https://cats.petiteweb.dev/api/single/WtfManOmg/delete/${catId}`, {
      method: 'DELETE',
    }).then((res) => {
      if (res.status === 200) {
        return $catWr.remove()
...
const openModalHandler = (e) => {
  ...
  if (targetModalName === 'createCat') {
    ...
    $createCatForm.addEventListener('submit', (submitEvent) => {
      ...
      fetch('https://cats.petiteweb.dev/api/single/WtfManOmg/add/', {
        method: 'POST',
        headers: {
